...
export default async function EpcistaDashboard() {
  ...
  const enAnalisis = lista.filter(p => p.estado === 'en_analisis').length
  const completados = lista.filter(p => p.estado === 'completado').length
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {lista.map((p, i) => (
                <tr
                  key={p.id}
                  style={{ backgroundColor: i % 2 === 0 ? '#fff' : '#fafaf8', borderTop: '1px solid #CFCFCF' }}
                >
