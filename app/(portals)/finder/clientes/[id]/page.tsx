...
export default function FinderDetalleClientePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (session?.user) setSessionUserId(session.user.id)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Link href="/finder/clientes" className="inline-flex items-center gap-1 text-sm mb-6 text-muted">
        <ChevronLeft size={14} />
        Volver a clientes
      </Link>
      ...
      {editando ? (
        ...
        <>
          ...
          {/* Usuarios del Cliente (Finder has read-only access) */}
          <UsuariosCliente
