...
import { Plus, MapPin, FileText, Pencil, Trash2, Upload, ExternalLink } from 'lucide-react'
  ...
  return (
    <div className="border p-6 mt-4" style={{ borderColor: '#CFCFCF', backgroundColor: '#fff' }}>
      ...
      <div className="flex flex-col gap-2 mb-4">
        {sitios.map(s => (
          <div key={s.id} className="border p-4" style={{ borderColor: '#CFCFCF' }}>
            {confirmDelete === s.id ? (
                  ...
                  <div className="flex flex-wrap gap-3 mt-1">
                    ...
                    {s.recibo_url && (
                      <a href={s.recibo_url} target="_blank" rel="noopener noreferrer"
                        ...
                        style={{ color: '#000' }}>
                        <FileText size={11} /> Ver recibo CFE <ExternalLink size={10} />
                      </a>
                    )}
                  </div>
                  {s.notas && <p className="text-xs mt-1.5 whitespace-pre-wrap" style={{ color: '#888' }}>{s.notas}</p>}
                </div>
                <div className="flex gap-1 ml-3 flex-shrink-0">
                  <button onClick={() => abrirEditar(s)} className="p-1.5 border" style={{ borderColor: '#CFCFCF' }}>
                    <Pencil size={12} />
                  </button>
                  <button onClick={() => setConfirmDelete(s.id)} className="p-1.5 border" style={{ borderColor: '#CFCFCF', color: '#c00' }}>
                    <Trash2 size={12} />
                  </button>
                </div>
              </div>
      ...
      {/* Formulario inline */}
      {mostrando && (
        <div className="border p-5" style={{ borderColor: '#000' }}>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              ...
              <input type="text" value={form.rpu} onChange={e => setForm(f => ({ ...f, rpu: e.target.value }))}
                className={inputClass} style={inputStyle} placeholder="Número de RPU CFE" />
            </div>
            <div>
