...
const emptyForm = { nombre: '', ciudad: '', ubicacion_estado: '', rpu: '', notas: '' }
...
export default function SitiosCliente({ clienteId, epcistaId, initialSitios }: Props) {
  ...
  // mostrando = true con editandoId = null → formulario nuevo sitio al fondo
  // mostrando = true con editandoId = string → edición inline del sitio correspondiente
  ...
  function abrirEditar(s: Sitio) {
    // Toggle: si ya estamos editando este sitio, cerrar
    if (editandoId === s.id && mostrando) {
      setMostrando(false)
      setEditandoId(null)
      return
    }
    setEditandoId(s.id)
    setForm({ nombre: s.nombre, ciudad: s.ciudad ?? '', ubicacion_estado: s.ubicacion_estado ?? '', rpu: s.rpu ?? '', notas: s.notas ?? '' })
  ...
  function toggleVer(id: string) {
    ...
    // Si abrimos ver, cerrar el form de edición de ese sitio
    if (editandoId === id && mostrando) {
      setMostrando(false)
      setEditandoId(null)
    }
  }
  ...
  // Formulario compartido (nuevo o editar inline)
  function FormSitio() {
    return (
        ...
        <div className="flex flex-col gap-3">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 110px', gap: '12px' }}>
            ...
            <div>
              ...
              <input type="text" value={form.rpu} onChange={e => setForm(f => ({ ...f, rpu: e.target.value }))}
      ...
      <div className="flex flex-col gap-2 mb-4">
        {sitios.map(s => (
          <div key={s.id}>
              ...
              {confirmDelete === s.id ? (
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setConfirmDelete(null)} className="px-3 py-1 text-xs border" style={{ borderColor: '#CFCFCF' }}>Cancelar</button>
                    <button onClick={() => eliminar(s.id)} className="px-3 py-1 text-xs font-bold text-white" style={{ backgroundColor: '#c00' }}>Eliminar</button>
                  </div>
                </div>
              ) : (
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button
                      onClick={() => toggleVer(s.id)}
                      className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border"
                      style={{ borderColor: viendoId === s.id ? '#000' : '#CFCFCF', backgroundColor: viendoId === s.id ? '#000' : '#fff', color: viendoId === s.id ? '#D7FF2F' : '#444' }}
                    >
                      <Eye size={12} /> Ver
                    </button>
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border"
                      style={{ borderColor: (editandoId === s.id && mostrando) ? '#000' : '#CFCFCF', backgroundColor: (editandoId === s.id && mostrando) ? '#f0f0f0' : '#fff', color: '#444' }}
                    >
            ...
            {viendoId === s.id && (
              <div className="border border-t-0 px-5 py-4" style={{ borderColor: '#000', backgroundColor: '#fafafa' }}>
                <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
                  ...
                  {!s.rpu && !s.recibo_url && !s.ciudad && !s.ubicacion_estado && !s.notas && (
            ...
            {/* Formulario editar — inline bajo el sitio */}
            {editandoId === s.id && mostrando && <FormSitio />}
          </div>
        ))}
      </div>

      {/* Formulario nuevo sitio — al fondo */}
